...
3. Дає задати список ключових слів (напр. `шахед`, `балістика`, `Дрони`).
...
| 2026-09-20 | **7.8** | **Зріз `к`/`г`/`х` тепер вимагає, щоб лишилося 4 символи, а не 3.** `танки` давало корінь `чай`, який є в будь-якому тексті й топив справжні тривоги. Зріз закінчення забирає лише граматику, а зріз приголосної перед ним — частину самого слова, і на короткому слові це інше слово. Тепер `танки` -> `танк`, а випадки, для яких правило й існує, живі: `Ставка` -> `став` (4), `Зенітка` -> `зеніт` (9). |
