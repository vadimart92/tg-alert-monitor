...
| 2026-09-05 | 7.8, R7 | Ключове слово автоматично зводиться до кореня, щоб покривати відмінки (`Білогородка` -> `білогород`). Рішення R7 (підрядок, без regex) збережено: стеммінг застосовується до **ключового слова** один раз при зміні списку, а не до тексту повідомлення, тому вартість пошуку не змінилася і на батарею це не впливає. Обрізається закінчення і `к`/`г`/`х` перед ним (чергування в місцевому відмінку: Білогородка -> у Білогородці). Слово, що вже закінчується на приголосну, не змінюється — це і є спосіб задати корінь вручну. Фрази з кількох слів не чіпаються. Корінь показано в UI, щоб автоматика не була непомітною. |
...
| 2026-09-20 | **7.8** | **Зріз `к`/`г`/`х` тепер вимагає, щоб лишилося 4 символи, а не 3.** `чайки` давало корінь `чай`, який є в будь-якому тексті й топив справжні тривоги. Зріз закінчення забирає лише граматику, а зріз приголосної перед ним — частину самого слова, і на короткому слові це інше слово. Тепер `чайки` -> `чайк`, а випадки, для яких правило й існує, живі: `Боярка` -> `бояр` (4), `Білогородка` -> `білогород` (9). |
