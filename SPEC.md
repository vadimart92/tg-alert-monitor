...
3. Дає задати список ключових слів (напр. `шахед`, `балістика`, `Київ`).
...
- SharedPreferences: `apiId`, `apiHash`, `botToken`, `targetChatId`, `maxAgeMinutes`, `alertCooldownSeconds`, `folderId`, `folderName`, `keywords`, `monitoringActive`, `cachedChats` (JSON), `phoneDisplay` (для показу, без коду).
...
- `<support>/keyword_sounds/kw_<hash>.wav` — згенерований голосовий сигнал ключового слова, `kw_<hash>.json` — фраза, якою він озвучений (`hash` = FNV-1a від нормалізованого слова). Не в SharedPreferences навмисно: аудіо і так файл, а файл, який сервіс просто `stat`-ить, завжди свіжий — нічого перезавантажувати й ні з чим синхронізувати.
...
| 2026-09-05 | 6.4 | Посилання на оригінал копіюється в буфер обміну по тапу (`url_launcher` не додано — специфікація дозволяє обидва варіанти). **Змінено 2026-09-05 (див. останній рядок).** |
...
| 2026-09-05 | 7.9 | На запит власника ключові слова в пересланому повідомленні виводяться як теги (`#зенітка`), а не через кому: так у цільовому каналі можна фільтрувати за конкретним словом. Пробіли й дефіси стають підкресленнями (`тест-ключ` -> `#тест_ключ`), бо Telegram обриває тег на них; слово, з якого не лишається нічого придатного для тега (лише цифри чи пунктуація), виводиться як екранований текст. |
...
| 2026-09-05 | 7.8, R7 | Ключове слово автоматично зводиться до кореня, щоб покривати відмінки (`зенітка` -> `зеніт`). Рішення R7 (підрядок, без regex) збережено: стеммінг застосовується до **ключового слова** один раз при зміні списку, а не до тексту повідомлення, тому вартість пошуку не змінилася і на батарею це не впливає. Обрізається закінчення і `к`/`г`/`х` перед ним (чергування в місцевому відмінку: зенітка -> у зенітці). Слово, що вже закінчується на приголосну, не змінюється — це і є спосіб задати корінь вручну. Фрази з кількох слів не чіпаються. Корінь показано в UI, щоб автоматика не була непомітною. |
| 2026-09-05 | 6.3 | Слова-винятки винесено в окремий розділ головного екрана з власним полем введення і поясненням, замість неочевидного префікса `-` у спільному списку. Формат зберігання не змінився (`-слово`), тож конфіг лишається сумісним. |
| 2026-09-05 | **R6**, 7.9 | **Рішення R6 скасовано на запит власника.** Оригінал тепер **пересилається** (`forwardMessages`) через сесію користувача, а не переписується текстом. Обґрунтування R6 («справжнє переслати неможливе») стосувалося **бота**: бот справді не має доступу до чужих каналів. Але застосунок і так тримає сесію користувача, який на ці канали підписаний, тож форвард від його імені працює — зі збереженням «Переслано з…» і медіа. Тег іде окремим коротким повідомленням під форвардом. |
| 2026-09-05 | 6.3, 7.9 | На запит власника доставку зроблено перемикачем на три положення (`AlertDelivery`: `forward` / `local` / `both`) на головному екрані. Канал у Telegram бачать інші люди, а сирена будить цей телефон — одне не заміняє інше, тому «Обидва» є повноцінним положенням, а не зручністю. У режимі `local` цільовий канал **не потрібен**: `isRunnable` вимагає його лише коли `delivery.forwards`, а форма налаштувань не блокує збереження через порожнє поле. Перемикання діє негайно через `monitor.config`. |
| 2026-09-05 | 7.9 | Локальне сповіщення показує `flutter_local_notifications` з окремого каналу `tg_alert_matches_v1`. Звук (`assets/siren_ostap_calm.ogg`) програє **сам Android** як звук каналу з `res/raw/`, а не аудіоплеєр у Dart: канал звучить навіть коли ізоляту сервісу пригальмовує система, і не потребує ще однієї плагін-залежності. Атрибути — `USAGE_ALARM`, щоб тривогу було чути в беззвучному режимі. Android заморожує звук і важливість каналу при створенні, тож id каналу версійований: нова сирена потребує нового id. Ізоляті сервісу передається лише колбек `LocalAlert`, тож `MonitorEngine` лишається без імпортів Flutter. |
| 2026-09-05 | 7.9 | Окреме повідомлення з тегом під форвардом прибрано на запит власника: воно подвоювало кількість дописів у цільовому каналі заради інформації, яка вже є в самому пересланому тексті. Теги (`MessageFormatter.renderKeywords`) тепер показано в журналі застосунку. Поле `ForwardTask.tag` видалено. |
| 2026-09-05 | 6.4 | У журналі збігів **тап** відкриває повідомлення в Telegram (`url_launcher`, `LaunchMode.externalApplication` — `t.me` перехоплює застосунок Telegram), **довге натискання** копіює посилання. Раніше тап копіював; відкриття потрібне частіше, ніж копіювання. Для Android 11+ у маніфест додано `<queries>` на `VIEW`/`https`, інакше система не показала б застосунку, хто вміє відкривати посилання. |
| 2026-09-05 | 6, R? | На запит власника інтерфейс локалізовано: українська **лишається** мовою за замовчуванням, англійська додана. `gen_l10n`, шаблон — `app_uk.arb` (українська є джерелом істини, англійська — переклад), мова береться з налаштувань пристрою. Згенерований `app_localizations*.dart` не тримаємо в git: його відтворює `flutter pub get`. |
| 2026-09-05 | 3, 7.9 | Ізолят сервісу не має `BuildContext`, тож `L.of(context)` там недоступний. `loadServiceStrings()` вантажить `L` напряму через `L.delegate.load()`, обираючи локаль за `PlatformDispatcher.instance.locale`; запасний варіант — українська, а не англійська, бо застосунок писався для українських користувачів. |
| 2026-09-05 | 3 | `MonitorEngine` мусить лишатися без імпортів Flutter (це те, що дозволяє тестувати його звичайним `flutter test`), а `L` — клас Flutter. Тому рядки рушія оголошені окремим інтерфейсом `EngineStrings` (чистий Dart) із типовою українською реалізацією; сервіс передає адаптер над `L`. Дублювання двадцяти рядків стереже тест `test/l10n_test.dart`, який звіряє їх з `app_uk.arb`. |
| 2026-09-05 | 6.1 | `ServiceBridge.initTask` перенесено з `App.initState` у `HomeScreen`: назва каналу сповіщень сервісу видима користувачеві, а `App` розташований **над** `MaterialApp`, тож локалізацій там ще немає. |
| 2026-09-05 | 6, 7.11 | На запит власника додано перенесення налаштування між телефонами через QR: перший показує код, другий сканує, підписується на канали, збирає їх у папку і вмикає режим «Локально». Канали в коді адресуються **`@username`, а не chat_id**: другий акаунт ніколи не бачив цих чатів, тож числовий id його TDLib не розв'яже. Наслідок — приватний канал у код не потрапляє взагалі; такі перелічуються на екрані обміну, щоб зникнення було видимим, а не тихим. |
| 2026-09-05 | 7.11 | Формат коду: `tgam1:` + base64url(gzip(JSON)) з однолітерними ключами. Gzip не косметичний: реалістичне налаштування (25 каналів, 40 слів) — це 3542 символи base64, що **перевищує місткість QR узагалі** (2953 байти); зі стисненням — 486. Кирилиця коштує два байти на символ у UTF-8, а ключі JSON повторюються. Префікс версіонований, щоб старіша збірка сказала «код новіший за мене», а не впала на невідомому полі. |
| 2026-09-05 | 6.3 | Сканування не застосовує код одразу: спершу екран підтвердження зі списком каналів і слів. Підписка на канали в чужому Telegram-акаунті — дія назовні, і робити її з кадру камери без питання не можна. Якщо телефон ще не в акаунті, код тримається до входу. |
| 2026-09-05 | 7.11 | Наявна папка з тим самим іменем **доповнюється**, а не замінюється (`getChatFolder` + `editChatFolder` з об'єднаним `included_chat_ids`): власник міг тримати в ній інші чати, і втратити їх через перенесення налаштування було б грубо. Один канал, що не розв'язався, не зриває решту — невдачі збираються і показуються в кінці. |
| 2026-09-05 | 6.2 | Кнопка «Відкрити my.telegram.org» і покрокова підказка на екрані налаштувань: знайти `api_id`/`api_hash` було найнеочевиднішим кроком першого запуску. Це саме передача в браузер — вхід там робить власник, значення копіює руками. |
| 2026-09-05 | **7.8 (баг)** | **Власний допис у власному каналі не спрацьовував узагалі.** `_handleNewMessage` відкидав усе з `is_outgoing == true`, а TDLib ставить цей прапорець на пости, зроблені власником. Тобто найприродніший спосіб перевірити застосунок — створити тестовий канал і написати туди — мовчав і на пересиланні, і на сирені. Перевірку звужено до **цільового чату**, для чого вона й існувала: не допустити петлі доставки. Тест на це в наборі був, але фіксував саме хибну поведінку, тож його переписано. |
| 2026-09-05 | 7.11 | `folders.chats` для **поточної** папки тепер не лише показує список, а й приймає його в роботу (`_adoptChats` + запис конфігу). Раніше щойно доданий у папку канал потрапляв під нагляд тільки через пів години, і кнопка «Показати чати» цьому не допомагала — вона показувала одне, а сервіс дивився на інше. |
| 2026-09-05 | **R6**, 7.9 | Бот **повернуто як необов'язковий**. Пересилання від імені власника лишається типовим, але має ваду, яку видно лише на другому пристрої: **повідомлення, надіслане вами, не сповіщає ваші ж інші пристрої**, тож власний форвард у цільовому каналі приходить беззвучно. Бот — інший відправник, тому його допис дає звичайне сповіщення. Бот не може переслати з каналу, де його немає, тож у режимі бота йде рендер (теги + текст + посилання), а не форвард. `bot_api.dart` відновлено з історії (коміт 87285f4^) без змін. |
| 2026-09-05 | 6.5 | Додано екран «Діагностика»: що саме під наглядом і коли список оновлювався, перевірка тексту справжнім матчером, перевірка сирени і перевірка доставки. Перевірка доставки навмисно йде **тим самим шляхом**, що й справжня тривога (`_deliverTask`), а не окремим тестовим кодом: інакше зелений результат нічого не доводив би. |
| 2026-09-05 | **8 (баг)** | **Сирена не звучала в release-збірці:** `PlatformException ... resource siren_ostap_calm could not be found`. На `res/raw/siren_ostap_calm.ogg` ніхто не посилається з коду чи XML — його шукають за іменем у рантаймі (`getIdentifier`), тож шринкер ресурсів AGP викидав його саме з release. У debug файл лишався, тому проблема була невидима до встановлення реальної збірки. Лікується `res/raw/keep.xml` з `tools:keep="@raw/siren_ostap_calm"`. Увага при перевірці: у release AGP ще й **скорочує шляхи** (`res/raw/siren_ostap_calm.ogg` -> `res/59.ogg`), тож шукати наявність за префіксом `res/raw/` у ZIP безглуздо — дивитися треба `aapt2 dump resources` на тип `raw`. |
| 2026-09-05 | **7.11 (баг)** | Оновлення папки записувало **весь** конфіг (`writeConfig`) з власної копії рушія, яка може бути старішою за щойно введене в UI. Раз на пів години це тихо скасовувало щойно додане ключове слово: у сховищі його вже не було, а головний екран далі показував його зі своєї копії в пам'яті. Тепер оновлення папки пише **лише список чатів** (`saveChats` -> `writeCachedChats`); увесь конфіг пишуть тільки шляхи, що йдуть від UI. |
| 2026-09-05 | 6.5 | Діагностику перенесено з меню QR у третю вкладку «Журналу»: питання «чому не спрацювало» і так починається із журналу, і тримати два різні місця для однієї відповіді безглуздо. Вкладка показує **список слів, який тримає сервіс** (з `diag.state`), а не той, що в пам'яті UI — саме розбіжність між ними і робила попередній баг невидимим. |
| 2026-09-05 | 7.9 | Канал сповіщень підвищено до `tg_alert_matches_v2`, старий видаляється. Android заморожує звук, важливість і аудіо-атрибути каналу при створенні й **ігнорує** пізніші зміни, а перша версія створювалася на збірці, де ресурсу сирени в APK не було — тобто ті встановлення тримають канал, звук якого так і не задався і не може бути виправлений на місці. Єдиний спосіб — новий id. |
| 2026-09-05 | 7.9 | Локальне сповіщення зроблено червоним (`color` + `colorized`, червоний LED, 🔴 у заголовку) і позначене `category: alarm`. Це тривога, і виглядати вона має як тривога, а не як звичайне сповіщення. |
| 2026-09-05 | 7.9 | Бот більше не повторює текст допису: він шле теги, час і **голе посилання**, а вміст показує власний прев'ю Telegram — з медіа, форматуванням і автором оригіналу. Раніше з вимкненим прев'ю виходила стіна тексту. `link_preview_options.show_above_text` ставить прев'ю над тегами. Виняток — приватне джерело: посилання `t.me/c/…` Telegram не розгортає нікому, тож для таких у повідомлення вкладається текст, інакше тривога не сказала б нічого. |
| 2026-09-05 | **7.7 (баг)** | **`getMessageLink` не працював жодного разу:** `TdError(400, Expected String, but receive Number)`. Одне з необов'язкових полів (`media_timestamp` / `checklist_task_id` / `poll_option_id` / `for_album` / `in_message_thread`) у 1.8.65 має тип, відмінний від того, що ми слали; яке саме — з бінарника не встановити. Тепер надсилаються **лише `chat_id` і `message_id`**, решту TDLib підставляє за замовчуванням, що нам і потрібно. Наслідок був не косметичний: кожна тривога падала на запасне посилання `t.me/c/…`, а його Telegram не розгортає — тобто щойно доданий прев'ю бота не працював би ніколи. |
| 2026-09-05 | 7.7 | Запасне посилання тепер двоступеневе: якщо `getMessageLink` недоступний, але канал публічний, будуємо `t.me/<username>/<id>` через `getSupergroup` (той самий шлях, що й для QR-перенесення). `t.me/c/…` лишається тільки для справді приватних чатів, де кращого не існує. |
| 2026-09-05 | **7.9** | **Сирену тепер програє застосунок, а не канал сповіщень.** Заміряно на Galaxy S10 (One UI, Android 12) з каналом рівно таким, як задумано: `mImportance=5`, `mSound=…/raw/siren_ostap_calm`, `usage=USAGE_ALARM`, DND вимкнено, гучність будильника 11/15, і `ringer mode affected streams` **не містить** STREAM_ALARM. У режимі «Без звуку» система однаково відмовляється програвати звук сповіщення. Ця відмова — вендорська, вище нашого каналу, і жодне налаштування каналу її не переконає. Пряме відтворення через `audioplayers` з `USAGE_ALARM` обходить її повністю. Канал сповіщення тепер **беззвучний** (`tg_alert_matches_quiet_v1`); якщо відтворення впаде, сповіщення показується на старому озвученому каналі — сирена, яку вендор може приглушити, все одно краща за жодної. |
| 2026-09-05 | **R6**, 4, 6.1 | **Бота прибрано повністю.** Оскільки доставка йде через сесію користувача, бот не потрібен ні для пересилання, ні для запасного текстового варіанта, ні для пошуку каналів. Видалено `BotApi`/`bot_api.dart`, залежність `http`, поле `botToken` і його UI. Кнопки перейменовано: «Перевірити канал» і «Тестове повідомлення» тепер перевіряють **справжній** шлях доставки, а не бота. Список цільових каналів фільтрується за правами **власника**, а не бота. Налаштування спростилися: BotFather і додавання бота адміністратором більше не потрібні. |
| 2026-09-20 | **7.8** | **Зріз `к`/`г`/`х` тепер вимагає, щоб лишилося 4 символи, а не 3.** `танки` давало корінь `тан`, який сидить усередині «стан» і топив справжні тривоги. Зріз закінчення забирає лише граматику, а зріз приголосної перед ним — частину самого слова, і на короткому слові це інше слово. Тепер `танки` -> `танк`, а випадки, для яких правило й існує, живі: `ставка` -> `став` (4), `зенітка` -> `зеніт` (5). |
| 2026-09-20 | **7.9** | **Ключове слово може мати власний голосовий сигнал.** Синтез — `flutter_tts` у UI-ізоляті, `synthesizeToFile(..., isFullPath: true)` у `<support>/keyword_sounds/`; сервіс лише **грає** готовий файл, тож повільний чи зламаний рушій синтезу не може ні затримати, ні зжерти тривогу. Для такого слова файл лунає **замість** сирени, тим самим `USAGE_ALARM`. Фраза редагується: шукане слово і вимовлене — не одне й те саме, бо ключове слово часто корінь. |
| 2026-09-20 | 7.9 | Рушій синтезу вибирається **нами, а не системним налаштуванням**: на Galaxy типовий — Samsung TTS, у якого української немає взагалі, а поруч стоїть Google TTS, у якого є. `KeywordVoice` питає типовий рушій, далі кожен інший установлений (`getEngines` + `setEngine`), і показує в UI, який саме рушій і мова озвучили файл. Для видимості рушіїв на Android 11+ у маніфест додано `<queries><intent><action android:name="android.intent.action.TTS_SERVICE"/>`. Окремий ризик: при помилці синтезу плагін **не завершує** свій `Future` — звідси таймаут і перевірка розміру файла. |
| 2026-09-20 | **7.9** | **У шторці тримається лише одна тривога, і живе вона 5 хв.** Один `id` замість `_nextId++` (нова заміняє попередню) плюс `timeoutAfter` і власний `Timer`: система знімає сповіщення навіть якщо процес убито, а таймер — навіть якщо прошивка проігнорувала прапорець. Наліт — це серія: п'ять повідомлень про той самий шахед за дві хвилини, і стос із них гірший за жодне, бо вранці треба прогорнути вчорашній наліт, щоб побачити, що зараз. |
| 2026-09-20 | **6.3**, 7.9 | **Режим «зачитувати текст повідомлення»** (`speakMessage`, типово вимкнено). Синтез — `flutter_tts` **у фоновому ізоляті сервісу**: плагін не `ActivityAware` і живе від `applicationContext`, тож працює без UI (перевірено на пристрої). Озвучення йде не через `tts.speak()`, а знову **в файл** (`<support>/alert_speech.wav`) з відтворенням через `audioplayers` на `USAGE_ALARM`: плагін уміє ставити лише навігаційні аудіо-атрибути, тобто `speak()` звучав би на музичному стрімі — рівно те, що ми обходили сиреною. Синтез стартує **перед** сиреною і триває, поки вона грає, тож голос не додає затримки; черговість тримає `onPlayerComplete`. Читається перші 300 символів. |
| 2026-09-20 | **7.9 (баг)** | `player.onPlayerComplete` оголошений як `Stream<void>`, але в рантаймі це `Stream<AudioEvent>`, тому `.first.timeout(onTimeout: () {})` кидає `type '() => Null' is not a subtype of '(() => FutureOr<AudioEvent>)?'` — аналізатор цього не бачить, падає лише на пристрої, і то в `catch`, тобто мовчки: сирена не грала, сповіщення тихо йшло на запасний канал. Лікується `.then<void>((_) {})` перед `timeout`. Знайдено тільки тому, що в логах каналу стояв `tg_alert_matches_v2` замість `_quiet_v1`. |
| 2026-09-20 | **6.1**, 7.7 | Паузу між тривогами для одного слова винесено в **налаштування** (`alertCooldownSeconds`, 0–3600 с, типово **30 с**), бо скільки повторів є шумом — залежить від каналів, які дивляться, і знає це лише власник. 30 хв, з якими це приїхало, на практиці виявилися завеликими: канал пише про один шахед двічі-тричі поспіль, а наступна подія — вже за кілька хвилин. `0` означає «озвучувати кожен збіг», тобто поведінку до появи паузи. Значення живе в `MonitorConfig`, тож іде тим самим шляхом, що й решта конфігу (`pushConfig` -> `updateConfig`), і діє одразу, без перезапуску моніторингу. У `AlertPolicy` лишилося тільки те, у чому немає судження, — 5 хв життя сповіщення. |
| 2026-09-20 | **7.7** | **Одне слово озвучується не частіше ніж раз на задану паузу** (спершу константа 30 хв, див. рядок вище; у рушії — `_lastAlertByKeyword` за нормалізованим словом). Затримується **лише звук на цьому телефоні**: збіг усе одно пишеться в журнал і пересилається в канал — серія це шум для власника, що спить, а не для каналу, де тривоги збирають. Тривога звучить, якщо хоч одне її слово вийшло з паузи, і ставить на паузу **всі** слова, які назвала (усі вони в тексті сповіщення). Сирена, яка **не спрацювала**, паузи не починає — її ніхто не чув. Слово, видалене і введене знову, чути одразу (`updateConfig` чистить мапу). У режимі «Локально» такий збіг отримує статус `muted` (🔇 у журналі) — інакше журнал брехав би «доставлено». |
| 2026-09-05 | 7.9 | `ForwardQueue` більше не знає про транспорт: приймає функцію доставки і зберігає лише чергу, інтервал 1.5 с і політику повторів. Помилки TDLib мапляться в `DeliveryFailure` (`FLOOD_WAIT_n` -> пауза, 400/403 -> без повторів). Канал із забороненим пересиланням (`CHAT_FORWARDS_RESTRICTED`) відкочується на текстову копію, щоб тривога не загубилася. |
